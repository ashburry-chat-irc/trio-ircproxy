...
menu Status,Channel {
  ...
  .-
  .$style_proxy &shutdown trio-ircproxy : /proxy-shutdown
  ...
  .-
  .$style_proxy u&se ip
  ..$iif(($varname_glob(bind-ip).value == private),$style(1)) 127.0.0.1 (private) : set $varname_global(bind-ip) private
  ..$iif(($varname_glob(bind-ip).value == global),$style(1)) [0.0.0.0 (global)] : set $varname_global(bind-ip) global
  ...
  ..change $block($varname_glob(use-port,proxy).value,/,$varname_glob(use-port,http).value) : {
    var %pp = $$?="enter two port numbers; for proxy and http [4321 80]:" 
...
alais style_proxy {
  if (!$varname_cid(trio-ircproxy.py,active)) { return $style(2) }
}
on *:text:*:?status: {
  ...
  if ($1 == use-port) { set $varname_global(use-port,proxy) $2 | set $varname_global(use-port,http) $3 }
